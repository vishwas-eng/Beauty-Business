...
function extractTextProperty(value: any): string {
  ...
  if (value.type === "multi_select") {
    return (value.multi_select ?? []).map((item: { name: string }) => item.name).join(", ");
  }
  ...
  if (value.type === "people") {
    return (value.people ?? []).map((item: { name?: string }) => item.name ?? "").join(", ");
  }

  if (value.type === "formula") {
    return value.formula?.string ?? String(value.formula?.number ?? "");
  }
...
export function mapNotionResults(
  results: any[],
  ...
  return results.map((result, index) => {
    ...
    return {
      ...
      title:
        extractTextProperty(properties[config.titleField]) ||
        extractTextProperty(properties.Name) ||
        "Untitled context",
      category:
        extractTextProperty(properties[config.categoryField]) ||
        extractTextProperty(properties.Category) ||
        "General",
      priority:
        extractTextProperty(properties[config.priorityField]) ||
        extractTextProperty(properties.Priority) ||
        "Normal",
      notes:
        extractTextProperty(properties[config.notesField]) ||
        extractTextProperty(properties.Notes) ||
        "No notes provided."
    };
  });
}
